...
uint8_t adns3080_read_reg(uint8_t addr)
{
    uint8_t tx_buf[2], rx_buf[2];

    tx_buf[0] = addr;
    HAL_SPI_TransmitReceive(&hspi2, tx_buf, rx_buf, 1, 10);
...
void adns3080_write_reg(uint8_t addr, uint8_t val)
{
    uint8_t tx_buf[2], rx_buf[2];
...
void adns3080_motion(motion_t *motion)
{
    ...
    HAL_SPI_Receive(&hspi2, rx_buf, 7, 10);
    __HAL_SPI_DISABLE(&hspi2);
